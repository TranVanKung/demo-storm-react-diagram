...
// import PathFinding from "node/routing/PathFinding";
...
export interface CustomLinkWidgetProps extends BaseWidgetProps {
  color?: string;
  width?: number;
  smooth?: boolean;
...
export interface CustomLinkWidgetState {
  selected: boolean;
}

export class CustomLinkWidget extends BaseWidget<
  CustomLinkWidgetProps,
  CustomLinkWidgetState
> {
  public static defaultProps: CustomLinkWidgetProps = {
    color: "black",
    width: 3,
    link: null,
    engine: null,
  ...
  render() {
    ...
    if (paths.length === 0) {
      if (points.length === 2) {
        ...
        var lastLinkIndex = points.length - 2;
        var extraParams;
        //draw the multiple anchors and complex line instead
        for (let j = 0; j < points.length - 1; j++) {
          if (j === firstLinkIndex) {
            extraParams = { startMarkerOnly: true };
          } else if (j === lastLinkIndex) {
